  ...
  Datum *_ppvar{};
   _ppvar = nrn_prop_datum_alloc(_mechtype, 6, _prop);
    ...
    assert(_nrn_mechanism_get_num_vars(_prop) == 16);
 	...
 	pbar = _parm_default[0]; /* 1e-05 */
 	a = _parm_default[1]; /* 0.21 */
 	 assert(_nrn_mechanism_get_num_vars(_prop) == 16);
 	_nrn_mechanism_access_dparam(_prop) = _ppvar;
 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  hoc_register_dparam_semantics(_mechtype, 5, "cvodeieq");
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 can /home/amajnas/wolf_model/imp_files/CaN.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "N-type Calcium Channel Current (CaN)";
...
static int  rates ( _internalthreadargsprotocomma_ double _lv ) {
 ...
 _lm_alpha = 0.1157 * ( _lv + 17.19 ) / ( exp ( ( _lv + 17.19 ) / 15.22 ) - 1.0 ) ;
   _lm_beta = 1.15 * exp ( _lv / 23.82 ) ;
   minf = 1.0 / ( 1.0 + exp ( - ( _lv + 8.7 ) / 7.4 ) ) ;
   mtau = 1.0 / ( _lm_alpha + _lm_beta ) ;
   htau = 23.33 ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 74.8 ) / 6.5 ) ) ;
    return 0; }
...
{ {
   ica = pbar * m * m * ( a * h + ( 1.0 - a ) ) * ghk ( _threadargscomma_ v , cai , cao ) ;
   }
 _current += ica;
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/home/amajnas/wolf_model/imp_files/CaN.mod";
