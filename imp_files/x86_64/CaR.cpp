 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  hoc_register_dparam_semantics(_mechtype, 5, "cvodeieq");
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 car /home/amajnas/wolf_model/imp_files/CaR.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "R-type Calcium Channel Current (CaR)";
...
static int  rates ( _internalthreadargsprotocomma_ double _lv ) {
 ...
 mtau = 1.7 ;
   minf = 1.0 / ( 1.0 + exp ( - ( _lv + 10.3 ) / 6.6 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + ( 33.3 ) ) / 17.0 ) ) ;
   if ( _lv <= - 30.0 ) {
     htau = 33.33 ;
     }
   else if ( _lv >= 10.0 ) {
     ...
     _lv_idx = floor ( ( _lv - ( - 30.0 ) ) / 10.0 ) ;
     _lfraction = ( ( _lv - ( - 30.0 ) ) / 10.0 ) - _lv_idx ;
     if ( _lv_idx  == 0.0 ) {
       htau = 33.33 + _lfraction * ( 21.67 - 33.33 ) ;
       }
     else if ( _lv_idx  == 1.0 ) {
       htau = 21.67 + _lfraction * ( 11.67 - 21.67 ) ;
       }
     else if ( _lv_idx  == 2.0 ) {
       htau = 11.67 + _lfraction * ( 10.0 - 11.67 ) ;
       }
     else if ( _lv_idx  == 3.0 ) {
       htau = 10.0 + _lfraction * ( 6.67 - 10.0 ) ;
       }
     }
    return 0; }
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/home/amajnas/wolf_model/imp_files/CaR.mod";
    const char* nmodl_file_text = 
  "TITLE R-type Calcium Channel Current (CaR)\n"
  "\n"
  "COMMENT\n"
  "Implements the R-type calcium channel (CaR) using the GHK current equation.\n"
  ...
  "- Kinetics: Activation time-constant mtau is fixed at 1.7 ms. Inactivation time-constant htau is \n"
  "  derived via linear interpolation of a hardcoded lookup table in the range of -30 mV to 10 mV.\n"
  "ENDCOMMENT\n"
  "\n"
  "NEURON {\n"
  "    SUFFIX car\n"
  "    USEION ca READ eca, cai, cao WRITE ica\n"
  "    RANGE pbar, ica, a, minf, hinf, htau, mtau\n"
  "}\n"
  "\n"
  "UNITS {\n"
  "    (mV) = (millivolt)\n"
