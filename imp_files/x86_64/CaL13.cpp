...
#define ecal _ml->template fpfield<8>(_iml)
#define ecal_columnindex 8
#define cali _ml->template fpfield<9>(_iml)
#define cali_columnindex 9
#define calo _ml->template fpfield<10>(_iml)
#define calo_columnindex 10
...
#define _ion_ecal *(_ml->dptr_field<2>(_iml))
#define _p_ion_ecal static_cast<neuron::container::data_handle<double>>(_ppvar[2])
...
static void nrn_alloc(Prop* _prop) {
 ...
 nrn_promote(prop_ion, 1, 1);
 	...
 	_ppvar[2] = _nrn_mechanism_get_param_handle(prop_ion, 0); /* ecal */
 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  register_nmodl_text_and_filename(_mechtype);
   ...
   _nrn_mechanism_register_data_fields(_mechtype,
                                       ...
                                       _nrn_mechanism_field<double>{"ecal"} /* 8 */,
                                       _nrn_mechanism_field<double>{"cali"} /* 9 */,
                                       _nrn_mechanism_field<double>{"calo"} /* 10 */,
                                       ...
                                       _nrn_mechanism_field<double*>{"_ion_ecal", "cal_ion"} /* 2 */,
  ...
  hoc_register_dparam_semantics(_mechtype, 5, "cvodeieq");
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 cal13 /home/amajnas/wolf_model/imp_files/CaL13.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "CaL 1.3 L-type Calcium Channel (Moczydlowski & Latorre 1983)";
...
static int  rates ( _internalthreadargsprotocomma_ double _lv ) {
   double _lm_alpha , _lm_beta , _lv_diff ;
 _lv_diff = _lv + 8.124 ;
   if ( fabs ( _lv_diff ) < 1e-6 ) {
     _lm_alpha = 0.1194 * 9.005 * ( 1.0 - _lv_diff / 9.005 / 2.0 ) ;
     }
   else {
     _lm_alpha = 0.1194 * _lv_diff / ( exp ( _lv_diff / 9.005 ) - 1.0 ) ;
     }
   _lm_beta = 2.97 * exp ( _lv / 31.4 ) ;
   mtau = 1.0 / ( _lm_alpha + _lm_beta ) ;
   minf = 1.0 / ( 1.0 + exp ( - ( _lv + 33.0 ) / 6.7 ) ) ;
   htau = 14.77 ;
...
static void _hoc_rates(void) {
 ...
 Datum* _ppvar; Datum* _thread; NrnThread* _nt;
  ...
  if(!_prop_id) {
    hoc_execerror("No data for rates_cal13. Requires prior call to setdata_cal13 and that the specified mechanism instance still be in existence.", NULL);
  }
...
{ {
   ical = pbar * m * m * h * ghk ( _threadargscomma_ v , cali , calo ) ;
   }
 _current += ical;
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/home/amajnas/wolf_model/imp_files/CaL13.mod";
    const char* nmodl_file_text = 
  "TITLE CaL 1.3 L-type Calcium Channel (Moczydlowski & Latorre 1983)\n"
  "\n"
  "COMMENT\n"
  ...
  "NEURON {\n"
  "    SUFFIX cal13\n"
  "    USEION cal READ cali, calo, ecal WRITE ical VALENCE 2\n"
  "    RANGE pbar, ical, minf, hinf, mtau, htau\n"
  "}\n"
  "\n"
  "UNITS {\n"
  "    (mV) = (millivolt)\n"
