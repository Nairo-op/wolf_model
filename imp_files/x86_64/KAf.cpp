 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 kaf /home/amajnas/wolf_model/imp_files/KAf.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "Fast A-type Potassium Channel Current (KAf)";
...
static int  rates ( _internalthreadargsprotocomma_ double _lv ) {
 ...
 minf = 1.0 / ( 1.0 + exp ( ( _lv - ( - 10.0 ) ) / - 17.7 ) ) ;
   htau = 4.67 ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv - ( - 75.6 ) ) / 10.0 ) ) ;
   if ( _lv <= - 40.0 ) {
     mtau = 0.6 ;
     }
   else if ( _lv >= 50.0 ) {
     ...
     _lv_idx = floor ( ( _lv - ( - 40.0 ) ) / 10.0 ) ;
     _lfraction = ( ( _lv - ( - 40.0 ) ) / 10.0 ) - _lv_idx ;
     if ( _lv_idx  == 0.0 ) {
       mtau = 0.6 + _lfraction * ( 0.367 - 0.6 ) ;
       }
     else if ( _lv_idx  == 1.0 ) {
       mtau = 0.367 + _lfraction * ( 0.333 - 0.367 ) ;
       }
     else if ( _lv_idx  == 2.0 ) {
       mtau = 0.333 + _lfraction * ( 0.333 - 0.333 ) ;
       }
     else if ( _lv_idx  == 3.0 ) {
       mtau = 0.333 + _lfraction * ( 0.3 - 0.333 ) ;
       }
     else if ( _lv_idx  == 4.0 ) {
       mtau = 0.3 + _lfraction * ( 0.267 - 0.3 ) ;
       }
     else if ( _lv_idx  == 5.0 ) {
       mtau = 0.267 + _lfraction * ( 0.3 - 0.267 ) ;
       }
     else if ( _lv_idx  == 6.0 ) {
       mtau = 0.3 + _lfraction * ( 0.3 - 0.3 ) ;
       }
     else if ( _lv_idx  == 7.0 ) {
       mtau = 0.3 + _lfraction * ( 0.3 - 0.3 ) ;
       }
...
static void _hoc_rates(void) {
 ...
 Datum* _ppvar; Datum* _thread; NrnThread* _nt;
  ...
  if(!_prop_id) {
    hoc_execerror("No data for rates_kaf. Requires prior call to setdata_kaf and that the specified mechanism instance still be in existence.", NULL);
  }
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/home/amajnas/wolf_model/imp_files/KAf.mod";
