...
#define _tsav _ml->template fpfield<12>(_iml)
  ...
  if (nrn_point_prop_) {
   ...
   _ppvar = nrn_prop_datum_alloc(_mechtype, 3, _prop);
    ...
    assert(_nrn_mechanism_get_num_vars(_prop) == 13);
 	...
 	tau1 = _parm_default[0]; /* 1.1 */
 	tau2 = _parm_default[1]; /* 5.75 */
 	e = _parm_default[2]; /* 0 */
  }
 	 assert(_nrn_mechanism_get_num_vars(_prop) == 13);
 	_nrn_mechanism_access_dparam(_prop) = _ppvar;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  register_nmodl_text_and_filename(_mechtype);
   ...
   _nrn_mechanism_register_data_fields(_mechtype,
                                       ...
                                       _nrn_mechanism_field<double>{"_tsav"} /* 12 */,
 ...
 pnt_receive_size[_mechtype] = 1;
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 AMPA /home/amajnas/wolf_model/imp_files/AMPA.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "AMPA Receptor-Mediated Excitatory Synaptic Current";
...
/*CVODE*/
 ...
 /*END CVODE*/
 static int state (_internalthreadargsproto_) { {
    m = m + (1. - exp(dt*(( - 1.0 ) / tau1)))*(- ( 0.0 ) / ( ( - 1.0 ) / tau1 ) - m) ;
    h = h + (1. - exp(dt*(( - 1.0 ) / tau2)))*(- ( 0.0 ) / ( ( - 1.0 ) / tau2 ) - h) ;
   }
  return 0;
}
...
{  Prop* _p; Datum* _ppvar; Datum* _thread; NrnThread* _nt;
 ...
 _tsav = t; {
    ...
    double __state = m;
    double __primary = (m + _args[0] * factor) - __state;
 ...
 m = m + _args[0] * factor ;
   ...
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = h;
    double __primary = (h + _args[0] * factor) - __state;
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/home/amajnas/wolf_model/imp_files/AMPA.mod";
    const char* nmodl_file_text = 
  "TITLE AMPA Receptor-Mediated Excitatory Synaptic Current\n"
  "\n"
  "COMMENT\n"
  "Implements an AMPA-receptor-mediated excitatory synaptic current using a \n"
  "dual-exponential conductance model.\n"
  "- Activation (rise) time constant: tau1 = 1.1 ms (default)\n"
  "- Inactivation (decay) time constant: tau2 = 5.75 ms (default)\n"
  ...
  "\n"
  "PARAMETER {\n"
  "    tau1 = 1.1 (ms)     : rise time (tau_on)\n"
  "    tau2 = 5.75 (ms)    : decay time (tau_off)\n"
  ...
  "    : weight - 0.000593 uS for a single-channel baseline\n"
  "    m = m + weight * factor\n"
  "    h = h + weight * factor\n"
  "}\n"
  ;
