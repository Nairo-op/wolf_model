  ...
  Datum *_ppvar{};
   _ppvar = nrn_prop_datum_alloc(_mechtype, 4, _prop);
    ...
    assert(_nrn_mechanism_get_num_vars(_prop) == 14);
 	...
 	gbar = _parm_default[0]; /* 0.0104 */
 	a = _parm_default[1]; /* 0.996 */
 	 assert(_nrn_mechanism_get_num_vars(_prop) == 14);
 	_nrn_mechanism_access_dparam(_prop) = _ppvar;
 ...
 hoc_register_parm_default(_mechtype, &_parm_default);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
    ...
    hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 kas /home/amajnas/wolf_model/imp_files/KAs.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static const char *modelname = "Slowly Inactivating A-type Potassium Channel Current (KAs)";
...
static int  rates ( _internalthreadargsprotocomma_ double _lv ) {
   double _lh_alpha , _lh_beta ;
 minf = 1.0 / ( 1.0 + exp ( ( _lv - ( - 27.0 ) ) / ( - 16.0 ) ) ) ;
   mtau = 0.378 + 9.91 * exp ( - pow ( ( ( _lv + 34.3 ) / 30.1 ) , 2.0 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv - ( - 33.5 ) ) / 21.5 ) ) ;
   _lh_alpha = 1.0 * exp ( - ( _lv + 90.96 ) / 29.01 ) ;
   _lh_beta = 1.0 * exp ( ( _lv + 90.96 ) / 100.0 ) ;
   htau = 1097.4 / ( _lh_alpha + _lh_beta ) ;
    return 0; }
...
static void _hoc_rates(void) {
 ...
 Datum* _ppvar; Datum* _thread; NrnThread* _nt;
  ...
  if(!_prop_id) {
    hoc_execerror("No data for rates_kas. Requires prior call to setdata_kas and that the specified mechanism instance still be in existence.", NULL);
  }
...
static void register_nmodl_text_and_filename(int mech_type) {
    const char* nmodl_filename = "/home/amajnas/wolf_model/imp_files/KAs.mod";
